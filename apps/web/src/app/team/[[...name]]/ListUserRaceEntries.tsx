...
// race: adult or Kids
// id: team ID
const ListUserRaceEntries = ({ race, id }) => {
  const { store } = useAuth();
  const [state, setState] = useState("INIT");
  const [password, setPassword] = useState("");
  const raceEntries = store.userStore.raceEntries.filter(
    (f) => f.race === race && !f.team
  );
  const onJoinTeam = async () => {
    if (id && password) {
      await store.userStore.joinTeam(
        store.userStore?.raceEntry?.id!,
        id,
        password
      );
    }
  ...
  return <ListRaceEntries raceEntries={raceEntries} />;
